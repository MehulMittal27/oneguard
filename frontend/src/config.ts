...
// How often to re-read the decision feed. The agent proposes purchases while
// the customer is looking at the app, so the feed has to catch up on its own —
// a screen that only updates on sign-in would quietly go stale.
//
...
export const DECISIONS_POLL_SECONDS = 5

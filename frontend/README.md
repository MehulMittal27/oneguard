...
serves now (D8), warning when one is already on record; hidden while `LIVE_RUNS_FROM_TERMINAL` in `ScenarioPanel.tsx`,
which says live runs start from the operator terminal, `make demo-live SCEN=<id>`), says whom to sign in as for the selected scenario and, on

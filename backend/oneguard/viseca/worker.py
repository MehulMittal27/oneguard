...
hold up the engine thread or the store threads (``_store``)."""
STANDBY_RETRY_S = 5.0
...
class VisecaWorker:
    ...
    async def stop(self) -> None:
        ...
        tasks = [
            t
            for t in [self._task, self._sync_task, self._bootstrap_task, *self._expiry.values(), *self._background, *self._rewrites]
            if t is not None
        ]
        for task in tasks:
            task.cancel()

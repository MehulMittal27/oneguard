...
reference data before D8 lists the catalogue, so `make demo-live` compiles the instruction the platform serves now. D2 replays only scenarios the local pack has purchases for (404 otherwise). D2 decides by the card's own policy, its words and checks:

...
| `cart.purchases_in_period` + `scope: period`, `period_days: N` | integer; purchases on this card in the rolling window of N×24h before the current simulated timestamp: **final approvals + pending step-ups** (declines and expired step-ups never count; a redelivered live id counts once). This purchase is compared as count + 1: `<= 1`, `period_days: 1` is "one a day", so a second purchase fails. Operators `<=` / `<` only. Fail: `period_count_exceeded`, message e.g. "You allowed one order per day; one was already approved today at 12:10" (time of the latest approval, Europe/Zurich); a breach caused only by pending step-ups asks (`period_reserved_pending`, M5) |

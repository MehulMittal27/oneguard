...
| D3 | POST | `/api/dev/runs` | `{ scenario_id, card_id, force?: boolean, policy?: 'card' \| 'scenario' }` | `LiveRun` - creates a Viseca run against the card's active mandate (`policy: 'scenario'`: the scenario's own instruction, registered for the run in place of the card's policy), followed by the worker; names the run card's holder |
...
starts nothing: 503 `upstream_unavailable` with the platform's `platform_status`, `platform_code` and `platform_message`, as any refused
...
**`policy: 'scenario'`** (the operator console's judging run; default `'card'`, the behaviour above). The platform
starts a run only when its mandate's instruction is the scenario's cardholder instruction exactly, else 409
`instruction_mismatch` ("The mandate instruction must exactly match the selected cardholder instruction"), so a card
whose policy the customer typed in other words cannot run it. With `policy: 'scenario'` D3 takes the scenario's
...
original is marked revoked (docs/decisions.md). D4 and D7 name the holder too, from the moment D3 answers (the card of the platform's reply, before the first purchase arrives).
...
LiveRun      { run_id, scenario_id, card_id, mandate_id, state: 'starting'|'running'|'done'|'error',
               ...
               platform_mandate?: {             // NEW: D3's check of the policy's mandate at the platform (§1.2)
                 ...
                 previous_viseca_mandate_id?: string,  // re-registered, or registered_for_run: the one it replaced
                 registered_for_run?: true,     //   NEW: D3 policy 'scenario': the scenario's instruction, registered
                                                //   for this run as the card's policy
                 replaced_mandate_id?: string } }  // NEW: registered_for_run: the card's policy it superseded
...
offline"). When the platform refused the call, its `detail` is `{ platform_status, platform_code,
platform_message? }`: the platform's HTTP status (null: it did not answer) and code, and (NEW) its own
message verbatim when it answered (the operator console shows it as is, §6 item 19).
`internal` (500) is an unexpected server error.
...
19. Operator console at `/ops` (desktop, operator only, never linked from the phone UI): a separate page of the same build (`src/ops/`), reading `/healthz`, D2, D3, D4, D5, D7, D8, D9, C6 (with `?operator=1`, §3.4) and C3's card, plus the passport endpoints (§1.3): the card's passport line (version, checks, devices, QR) is checked with P5, and each decision with a `receipt_id` has its receipt (P4) checked with P5. It follows D7: whatever run started last, from any source, is the one its run header, stream and embedded phone show while it is in progress (starting, running, or with step-ups waiting); once it is not, the run header says "No run in progress." and shows that finished run, labelled finished, only while its scenario is selected; a line under the buttons names it ("Last run: Replay from record · SCEN0101 · Omar Chen (CU1217, card CA1331) · running"), apart from the "Sign in as <name> (<customer id>, card <card id>)" line, which follows the selected scenario's customer (D9). The embedded phone is signed in as the shown run's customer, else as the selected scenario's customer; its iframe is keyed on its URL, so another customer is a fresh app and nothing of the previous customer's cards (passport, devices, this device's role) stays on screen. Its Replay buttons (D2) replay a served scenario from record, saying so under them and in the run header ("Replay from record", with the recorded run's start); for a scenario D9 marks `replay_source: null` they are disabled with "Not run yet: no stored events to replay." Its "Judging run (live)" (D3, sent with `policy: 'scenario'`) is disabled, with the reason on the button and under it, for a scenario D8 does not mark `served` ("Replay only: not served by the sandbox.", said first: no worker state changes it), and while `/healthz` shows the worker off (`worker.configured` false) or not polling (`worker.state` other than `polling`), or has not answered, or D8 or the runs on record have not been read. A served scenario with a finished live run on record (the card's `live-<id>` runs in C6, each read with D4 for its `scenario_id` and `state: done`) keeps the button enabled with a warning, "Already on record (run <start>)", repeated in the typed-id confirmation. The typed-id confirmation says "This run uses the scenario's own instruction; the card's current policy will be replaced." A refusal shows D3's message and, when the platform refused (409 `instruction_mismatch`, …), the platform's own words under it: "Platform <status> <code>: <platform_message>", the message verbatim. It never answers a step-up and never enrols, approves or removes a device; the customer does that on the phone. It wears Viseca's colours and type (Roboto via Google Fonts, system fallback); the embedded phone keeps its own. The run header names a replay's policy from D7's `policy_source`: "the card's active policy", "policy revoked: every purchase declines" or "policy compiled from the scenario"; for a live run whose `platform_mandate.reregistered` is true it adds "platform mandate re-registered" (its tooltip names both platform mandates and the status the old one had), and for one whose `platform_mandate.registered_for_run` is true "policy: the scenario's instruction, registered for this run" (its tooltip names the platform mandate and the policy it replaced, and that the customer did not confirm it). Its left column has three tabs: Overview (scenario, instruction, buttons, run header, the last 5 decisions), Decision log (the run's stream, filtered by outcome and shop, rows expandable, Export JSON of the visible rows) and Health (the `/healthz` body as a table, with a copy button).

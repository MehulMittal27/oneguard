...
Add Dockerfile (python:3.12-slim, install backend, copy frontend/dist built in a node stage, ONEGUARD_SOFT_SIGNALS=keywords default in the image, uvicorn on $PORT), fly.toml (region lhr, one machine, no volume — state lives in Supabase via ONEGUARD_DATABASE_URL; secrets VISECA_API_KEY OPENAI_API_KEY ONEGUARD_DATABASE_URL), `make deploy`. /healthz reports worker polling, provider configured, signals backend, database engine (sqlite/postgres) and a 1-row round-trip time. Warm the DB pool at startup. Deploy with a null key; verify the placeholder/UI loads and D2 replay fills C6 on the cloud.
```

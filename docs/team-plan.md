...
7. Create backend/oneguard/store/: db.py (SQLAlchemy 2.x engine factory from ONEGUARD_DATABASE_URL, default sqlite:///./oneguard.sqlite, psycopg for postgresql+psycopg://, pool_pre_ping, session() context manager, init_db()), schema.py (every table in docs/database.md §2 with the JSON/DateTime(timezone=True)/Numeric(12,2) conventions and the listed indexes), seed.py (idempotent CSV → reference tables, verifies metadata.json hashes; `make seed`; `make reset-db` guarded by ONEGUARD_ENV != prod), history.py (HistoryIndex implementation per docs/database.md §3: customer-level and card-level familiarity, refunds and cash withdrawals excluded, max_approved, last_price, recent_rows, merchant_names_normalised). Add `sqlalchemy` and `psycopg[binary]` to pyproject dependencies. engine/ledger_base.py's Ledger takes a store session.
...
**Prompt P5-2 — protections + warning signs** *(A1–A7 and W1–W5 as pure signal functions over Facts + LedgerView)*
```text

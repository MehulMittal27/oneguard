...
| `passports` | P1 passport | `(passport_id, version)` PK, `mandate_id`, `card_id`, `customer_id`, `document` JSON (the signed body), `signature`, `key_id`, `issued_at`, `reason` (confirmed, backfill, tightened, devices, controller, confirmation, revoked, updated), `superseded_at`, `revoked_at`. `passport_id` is derived from the mandate id (one per mandate); latest = highest version. A new version only when what it says changed; a revoked one never changes again |

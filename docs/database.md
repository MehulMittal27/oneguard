...
- **SQLite runs in WAL mode** (`store/db.py`, set on every connection). The worker writes
  from several threads at once (call log, `events_raw`, `runs`, the cursor, the ledger);
  under the rollback journal each commit locked out readers and writers and fsynced the
  file, and on a loaded machine the stalls added up past the decision deadline. The
  `oneguard.sqlite-wal` and `-shm` files next to the database are part of it while a
  process has it open; copy the database only when no process does.
...
| `mandates` | P1 api (C2, C4, C5) | `mandate_id` PK (ours), `viseca_mandate_id`, `card_id`, `customer_id`, `instruction`, `rules` JSON, `checks` JSON, `uncertainty_policy`, `open_questions` JSON, `status` (active/revoked/superseded: an operator's judging run replaced it with the scenario's instruction, D3 `policy: 'scenario'`), `confirmed_at`, `revoked_at` (when it ended, either way), `note` nullable (why a policy began or ended other than by the customer's own act: "Replaced by md_… for the operator's judging run of SCEN…", "Registered for the operator's judging run of SCEN…, without the customer's confirmation.") |
| `runs` | P1 worker / api offline replay | `run_id` PK (ours: `live-<viseca run>`, `replay-<hex>`), `viseca_run_id` nullable, `kind` (live/replay; a step-up in a `replay` run is closed locally, any other through the worker), `scenario_id`, `mandate_id`, `card_id`, `state`, counters, `started_at`, `finished_at`, `worker_last_poll_at`, `last_error`, `record_run_id` (nullable: for a replay from record, the `live-<viseca run>` whose stored `events_raw` it replays; D2, api-contract.md §1.2; added in place by `init_db`), `platform_mandate` JSON (nullable: for a live run D3 started, D3's check of the policy's mandate at the platform, `LiveRun.platform_mandate`, so D4 and D7 show it after a restart; added in place by `init_db`). The worker writes the row (`kind = live`) before a run's first decision; a live row's counters are recomputed from `events_raw` and `decisions` on every write and `started_at` keeps its first write, so a restart or a second process never resets them; the lease holder writes every run's row once more when it stops, so a stop between a decision and the loop's next row write leaves the row current; the offline replay (D2, and `make replay` through the same `OfflineRunner`) writes its `replay` row before its first: the ledger carries the session watch and remembered answers over only between live runs, and a run with no row counts as a replay |
...
| `devices` | P1 passport | `device_id` PK (uuid, returned by enrolment), `card_id`, `customer_id`, `label`, `public_key_jwk` (P-256, public members only), `status` (pending/enrolled/removed), `enrolled_at`, `enrolled_by_device_id` (null for the card's first device), `removed_at`, `last_seen_at`, `controller_since` (the card's controller is the enrolled device with the latest one; NULL on older rows: the earliest enrolled). Index `(card_id, status)`. Rows are never deleted; the operator reset marks them removed |
...
| `passports` | P1 passport | `(passport_id, version)` PK, `mandate_id`, `card_id`, `customer_id`, `document` JSON (the signed body), `signature`, `key_id`, `issued_at`, `reason` (confirmed, backfill, tightened, devices, controller, confirmation, platform, revoked, updated), `superseded_at`, `revoked_at`. `passport_id` is derived from the mandate id (one per mandate); latest = highest version. A new version only when what it says changed; a revoked one never changes again |
...
| `worker_events` | P1 client / worker / D3 | `id` PK, `at`, `kind`, `action`, `status`, `code`, `message` (≤ 2 KB), `mandate_id`, `run_id`, `authorization_id`, every column but `id`, `at`, `kind` nullable. What must outlive Fly's log buffer: `platform_refusal`, every Viseca call that raised `VisecaError` (a non-2xx answer: its status, code and response body; or no answer: status null, `upstream_unavailable`), written by the client's `refusal_sink` off the event loop, whatever the call (`action`: the client method, e.g. `create_run`, `create_mandate`, `post_decision`); `mandate_inactive`, the platform's event snapshot reporting our mandate other than active during a run (`code` its status, once per mandate and status); `mandate_reregistered`, D3 registering a policy again (`mandate_id` the new platform mandate, `message` naming the old). Each insert keeps the newest 200 of its kind (`store/worker_events.py`). Never a header or the key (the client scrubs it from every message). `/healthz` shows the newest refusal (`last_refusal`). A new table, created by `init_db` on an existing store |
...
`devices.controller_since` nullable (when the device became the card's controller; NULL on
rows from before controllers, which read the earliest enrolled device as the controller,
docs/passport.md §4).

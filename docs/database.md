...
| `decision_checkpoints` | `pipeline.py` via `checkpoints.sink`, once per new decision | `live_authorization_id` PK, `checkpoints` JSON (api-contract §3.10), `recorded_at`. Own table so `decisions` keeps its columns; `create_all` adds it to an existing store |

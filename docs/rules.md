...
| A8 | Already bought | The instruction asks for its requested item once ("the X I chose", "one X", "a X": `Policy.single_item`), the cart holds it, and a final approval under the same mandate already bought it (an approval, or a step-up the customer approved; declines, pending, declined and expired step-ups fulfil nothing, so a re-quote of a declined purchase is another only when something else was bought). Live runs under the same mandate share it; replays keep their own. | Ask (`already_fulfilled`), after the other protections that ask: "You already bought the X on 12 Aug for CHF 289.00; approve another?" A broken rule still declines first. |
...
|---|---|---|---|
| Q1 | One purchase or several per policy? | **Closed (superseded 2026-09-25).** A single-item instruction ("the X I chose", "one X", "a X") is fulfilled by its first final approval; a later purchase of that item asks (A8, `already_fulfilled`). Any other instruction: each purchase judged on its own; only near-identical repeats caught (A3). | Live SCEN0122: the chosen camera lens was bought 8 times (decisions.md 2026-09-25). AU0023 (road-running shoes, not one item) stays compliant; AU0042 is still a legitimate re-quote, of a monitor already bought, so it asks. |

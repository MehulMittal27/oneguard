...
run has a provider; D5 off clears it): the model call runs off the engine thread with an
8 s timeout, then one short ledger session stores the new message with
